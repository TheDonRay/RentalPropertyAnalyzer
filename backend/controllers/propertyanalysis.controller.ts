...
const PropertyAnalysisController = async (Address : clientAddress, req: Request, res: Response)  => { 
    ...
    try { 
        ...
        return res.status(200).send(`Data Successfully Analyzed here’s what we think about the property based of the data provided: ${aiPropertyAnalysis.choices[0].message.content}`); 
    } catch (error) { 
